...
def output():
    ...
    '''
    # Write data
    print ("Write data", file = f)
    io.write_to_file('../../data/output/out9.txt', environment)
    # Create a GIF from the images
    imageio.mimsave('../../data/output/out9.gif', images, fps=3) # 3 frames per second
...
def update(frames):
    '''
    Move each agent, allow it to eat, then calculate the agent store and the remaining environment.
    Share the agent stores
    ...
    #print("Iteration", frames, file = f)
    
    # Move agents
    print("Move and Eat", file = f)
    for i in range(n_agents):
    ...
    print("Maximum distance between all agents", get_max_distance(agents), file = f)
    ...
    if sum_as / n_agents > 80:
        carry_on = False
        print("Stopping Condition: average agent store = ", sum_as / n_agents, file = f)
        print("Iterations Run = ", frames, file = f)
...
def gen_function():
    '''
    Pass the iteration variable back whilst continuing to run the while loop.
    Write the output data to a file, and the images to a GIF
...
if __name__ == '__main__':
    ...
    # Open a file to print the output to
    f = open('../../data/output/console_op.txt', 'w')
    ...
    for i in range(n_agents):
        ...
        print(agents[i].agents[i], file = f)
...
#        update(ite)    
    ...
    #print("Time take to calculate max distance", end - start, "seconds",file = f)
    #print("Maximum distance between agents at initialisation", maximum_distance, file = f)
